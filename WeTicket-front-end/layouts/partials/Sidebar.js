...
const { about, most_sale_tickets } = config.widgets;
  ...
  return (
    <aside className={`${className} px-0 lg:px-6 lg:col-4`}>
      ...
      {most_sale_tickets.enable && (
        <div className="mt-6 rounded border border-border p-6 dark:border-darkmode-border">
          <h4 className="section-title mb-12 text-center">Tickets</h4>
          ...
          {showRecent
            ? sortPostByDate
                .slice(0, most_sale_tickets.showPost)
                .map((post, i, arr) => (
                  <div
